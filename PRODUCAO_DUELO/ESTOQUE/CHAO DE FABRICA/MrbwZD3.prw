...
Local GpUsrAlm 	  := GetNewPar("US_GRPALMX","000000") 
Local GrUsrPrd 	  := GetNewPar("US_GRPPROD","000000") 


IF ALLTRIM(ZD3->ZD3_STATUS) == "1" .AND.   _cCdUser $ GrUsrPrd
	...
	ELSE
		Return
	ENDIF
ELSEIF ALLTRIM(ZD3->ZD3_STATUS) == "2" .AND.  _cCdUser $ GpUsrAlm
	IF MsgYesNo("Confirma Aprovação?")
		DBSELECTAREA("ZD3")
		DBSETORDER(1)
		IF DBSEEK(xFilial("ZD3")+ZD3->ZD3_NUMSEQ)
