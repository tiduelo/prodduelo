...
#DEFINE MAXITEMC  056 //012                                          // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN 135 //130                                          // Máximo de caracteres por linha de dados adicionais
...
#DEFINE CLR_WHITE RGB(0,0,0)											//Inversão de cores
#DEFINE CLR_BLACK RGB(225,225,225)										//Inversão de cores
...
//	    nVUnit   := IIF(MV_PAR07 == 2, Val(oDet[nX]:_Prod:_vUnCom:TEXT),Val(oDet[nX]:_Prod:_vUnTrib:TEXT)) // RAFAEL ALMEIDA - SIGACORP VALIDAÇÃO IMPRESSAO VALOR CONTRIBUTARIO // DESATIVANDO VERSAO 12
	    nVUnit   := IIF(MV_PAR09 == 2, Val(oDet[nX]:_Prod:_vUnCom:TEXT),Val(oDet[nX]:_Prod:_vUnTrib:TEXT)) // RAFAEL ALMEIDA - SIGACORP VALIDAÇÃO IMPRESSAO VALOR CONTRIBUTARIO 
	EndIf

//	nQtd     := IIF(MV_PAR07 == 2 , Val(oDet[nX]:_Prod:_qCom:TEXT),Val(oDet[nX]:_Prod:_qTrib:TEXT)) // RAFAEL ALMEIDA - SIGACORP VALIDAÇÃO IMPRESSAO VALOR CONTRIBUTARIO  // DESATIVANDO VERSAO 12
	nQtd     := IIF(MV_PAR09 == 2 , Val(oDet[nX]:_Prod:_qCom:TEXT),Val(oDet[nX]:_Prod:_qTrib:TEXT)) // RAFAEL ALMEIDA - SIGACORP VALIDAÇÃO IMPRESSAO VALOR CONTRIBUTARIO 
...
/*
//	AADD(aAux, AllTrim(TransForm(nDesc,TM(nDesc,TamSX3("D2_DESCON")[1],TamSX3("D2_DESCON")[2]))))       //Rafa 4
//	AADD(aAux, AllTrim(TransForm(nVUniLiq,TM(nVUniLiq,TamSX3("D2_PRCVEN")[1],4))))						//Rafa 5
//	AADD(aAux, AllTrim(TransForm(nVTotLiq,TM(nVTotLiq,TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2]))))   //Rafa 6
*/
	aAux := {}
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_TOTAL")[2]))))
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_TOTAL")[2]))))
...
/*
		SubStr(aAux[4], 1, PosQuebrVal(aAux[4])),;//Rafa 4
		SubStr(aAux[5], 1, PosQuebrVal(aAux[5])),;//Rafa 5
		SubStr(aAux[6], 1, PosQuebrVal(aAux[6])),;//Rafa 6
*/
	...
	aadd(aItens,{;
		...
		IIF(MV_PAR09 == 2,oDet[nX]:_Prod:_uCom:TEXT,oDet[nX]:_Prod:_utrib:TEXT),;//		IIF(MV_PAR07 == 2,oDet[nX]:_Prod:_uCom:TEXT,oDet[nX]:_Prod:_utrib:TEXT),;  // DESATIVANDO VERSAO 12
...
*/
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]);
	       .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6]) .Or. !Empty(aAux[7]) .Or. !Empty(aAux[8])
...
For nX :=1 To nLenItens    //rafael 29/11/2016 analisar os itens para impressao 
	nY++
...
For nY := 1 To nLenItens
	...
	Else //imprimi valores dos itens   

/*
			oDanfe:Say(nLinha, 100 + 2, aAux[1][1][nY], oFont07:oFont)//102 //Cod Produto
			oDanfe:Say(nLinha, 145 + 2, aAux[1][2][nY], oFont07:oFont)//147 //Desc Produto
			oDanfe:Say(nLinha, 356 + 2, aAux[1][3][nY], oFont07:oFont)//359 //Ncm
			oDanfe:Say(nLinha, 391 + 2, aAux[1][4][nY], oFont07:oFont)//398 //Cst
			oDanfe:Say(nLinha, 410 + 2, aAux[1][5][nY], oFont07:oFont)//417 //Cfop
			oDanfe:Say(nLinha, 433 + 2, aAux[1][6][nY], oFont07:oFont)//440 //Un  
*/
 

			oDanfe:SayAlign(nLinha-8, 100 + 2, aAux[1][1][nY], oFont07:oFont,45, 10, , 0, )
			oDanfe:SayAlign(nLinha-8, 145 + 2, aAux[1][2][nY], oFont07:oFont,300, 10, , 0,)
			oDanfe:SayAlign(nLinha-8, 356 + 2, aAux[1][3][nY], oFont07:oFont,52, 10, , 0, )
			oDanfe:SayAlign(nLinha-8, 391 + 2, aAux[1][4][nY], oFont07:oFont,30, 10, , 0, )
			oDanfe:SayAlign(nLinha-8, 410 + 2, aAux[1][5][nY], oFont07:oFont,30, 10, , 0, )
			oDanfe:SayAlign(nLinha-8, 433 + 2, aAux[1][6][nY], oFont07:oFont,30, 10, , 0, )
			

//			nAuxH2 := len(aAux[1][7][nY]) + (aColProd[7][1] + ((aColProd[7][2] - aColProd[7][1]) - RetTamTex(aAux[1][7][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 453 + 2, aAux[1][7][nY], oFont07:oFont)//453 //480
			oDanfe:SayAlign(nLinha-8, 449 + 2, aAux[1][7][nY], oFont07:oFont,45, 10, , 1, )//QUANT			

//			nAuxH2 := len(aAux[1][8][nY]) + (aColProd[8][1] + ((aColProd[8][2] - aColProd[8][1]) - RetTamTex(aAux[1][8][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 499 + 2, aAux[1][8][nY], oFont07:oFont) //499 //529
			oDanfe:SayAlign(nLinha-8, 490 + 2, aAux[1][8][nY], oFont07:oFont,52, 10, , 1, )//V UNITARIO
		
//			nAuxH2 := len(aAux[1][9][nY]) + (aColProd[9][1] + ((aColProd[9][2] - aColProd[9][1]) - RetTamTex(aAux[1][9][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 547 + 2, aAux[1][9][nY], oFont07:oFont)//547	//571 
			oDanfe:SayAlign(nLinha-8, 538 + 2, aAux[1][9][nY], oFont07:oFont,50, 10, , 1, )//V. TOTAL			
		                                                                            
//			nAuxH2 := len(aAux[1][10][nY]) + (aColProd[10][1] + ((aColProd[10][2] - aColProd[10][1]) - RetTamTex(aAux[1][10][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 593 + 2, aAux[1][10][nY], oFont07:oFont)    //593  //614   
			oDanfe:SayAlign(nLinha-8, 574 + 2, aAux[1][10][nY], oFont07:oFont,50, 10, , 1, )//BC. ICMS
		
//			nAuxH2 := len(aAux[1][11][nY]) + (aColProd[11][1] + ((aColProd[11][2] - aColProd[11][1]) - RetTamTex(aAux[1][11][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 630 + 2, aAux[1][11][nY], oFont07:oFont)   //630  //660
			oDanfe:SayAlign(nLinha-8, 620 + 2, aAux[1][11][nY], oFont07:oFont,50, 10, , 1, )//BC. ICMS ST			
		
//			nAuxH2 := len(aAux[1][12][nY]) + (aColProd[12][1] + ((aColProd[12][2] - aColProd[12][1]) - RetTamTex(aAux[1][12][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 675 + 2, aAux[1][12][nY], oFont07:oFont) //  675 //700
			oDanfe:SayAlign(nLinha-8, 665 + 2, aAux[1][12][nY], oFont07:oFont,45, 10, , 1, )//V. ICMS
		
//			nAuxH2 := len(aAux[1][13][nY]) + (aColProd[13][1] + ((aColProd[13][2] - aColProd[13][1]) - RetTamTex(aAux[1][13][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 715 + 2, aAux[1][13][nY], oFont07:oFont)  //  715   //751			
			oDanfe:SayAlign(nLinha-8, 715 + 2, aAux[1][13][nY], oFont07:oFont,45, 10, , 1, )//V. ICMS ST
		
//			nAuxH2 := len(aAux[1][14][nY]) + (aColProd[14][1] + ((aColProd[14][2] - aColProd[14][1]) - RetTamTex(aAux[1][14][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 765 + 2, aAux[1][14][nY], oFont07:oFont) //  765  //789			
			oDanfe:SayAlign(nLinha-8, 757 + 2, aAux[1][14][nY], oFont07:oFont,45, 10, , 1, )//V. IPI
		
//			nAuxH2 := len(aAux[1][15][nY]) + (aColProd[15][1] + ((aColProd[15][2] - aColProd[15][1]) - RetTamTex(aAux[1][15][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 807 + 2, aAux[1][15][nY], oFont07:oFont)  //  806     //817
			oDanfe:SayAlign(nLinha-8, 805 + 2, aAux[1][15][nY], oFont07:oFont,30, 10, , 1, )//A.ICMS
		
//			nAuxH2 := len(aAux[1][16][nY]) + (aColProd[16][1] + ((aColProd[16][2] - aColProd[16][1]) - RetTamTex(aAux[1][16][nY], oFont07:oFont, oDanfe)))
//			oDanfe:Say(nLinha, 840 + 2, aAux[1][16][nY], oFont07:oFont) //  840	 //847
			oDanfe:SayAlign(nLinha-8, 836 + 2, aAux[1][16][nY], oFont07:oFont,30, 10, , 1, )//A.IPI
...
Else
	...
	oDanfe:Say(MAXBOXV-10, INIBOXH+30, "DATA DE EMISSÃO:   "+Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oIdent:_DHEmi:TEXT),ConvDate(oIdent:_DEmi:TEXT))+Space(50)+"VALOR NF:     "+ TRansform(Val(oNFe:_NFE:_INFNFE:_TOTAL:_ICMSTOT:_VNF:TEXT),"@E 999,999,999.99") , oFont07:oFont, , , 270 )
EndIf
...
//Duelo - Aparecer somente uma Natureza na Descição da Operação
_nPosNat := At("/", oIdent:_NATOP:TEXT)

If _nPosNat > 0
	_nPNatOp := _nPosNat - 1
	_cNatOpe := SubStr(oIdent:_NATOP:TEXT,1,_nPNatOp)
	oDanfe:Say(nLine+148,nBaseTxt,"NATUREZA DA OPERAÇÃO",oFont08N:oFont)
	oDanfe:Say(nLine+158,nBaseTxt,_cNatOpe,oFont08:oFont)
Else
	oDanfe:Say(nLine+148,nBaseTxt,"NATUREZA DA OPERAÇÃO",oFont08N:oFont)
...
If nFolha == 1
	...
	//  oDanfe:Box(nLine+452,nBaseCol+30,nLine+575,MAXBOXH+70)
	nAux := nBaseCol + 30
	oDanfe:Box(nLine+452,nBaseCol+30,nLine+575,MAXBOXH+70)
	
	oDanfe:Box(nLine+452, 100, nLine+469, 145)
	oDanfe:Say(nLine+462, 100 + 2,"COD. PROD", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 145, nLine+469, 356)
	oDanfe:Say(nLine+462, 145 + 2,"DESCR PROD", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 356, nLine+469,391 )
	oDanfe:Say(nLine+462, 356 + 2,"NCM/SH", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 391, nLine+469, 410)
	If cMVCODREG == "1"
		oDanfe:Say(nLine+462, 391 + 2,"CSOSN", oFont08N:oFont)
	Else
		oDanfe:Say(nLine+462, 391 + 2,"CST", oFont08N:oFont)
	Endif
	
	oDanfe:Box(nLine+452, 410, nLine+469, 433)
	oDanfe:Say(nLine+462, 410 + 2,"CFOP", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 433, nLine+469, 453)
	oDanfe:Say(nLine+462, 433 + 2,"UN", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 453, nLine+469, 499)
	oDanfe:Say(nLine+462, 453 + 2,"QUANT.", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 499, nLine+469, 547)
	oDanfe:Say(nLine+462, 499 + 2,"V.UNITARIO", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 547, nLine+469, 593)
	oDanfe:Say(nLine+462, 547 + 2,"VLR TOTAL", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 593, nLine+469, 630)
	oDanfe:Say(nLine+462, 593 + 2,"BC.ICMS", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 630, nLine+469, 675)
	oDanfe:Say(nLine+462, 630 + 2,"BC.ICMS ST", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 675, nLine+469, 715)
	oDanfe:Say(nLine+462, 675 + 2,"VLR ICMS", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 715, nLine+469, 765)
	oDanfe:Say(nLine+462, 715 + 2,"VLR ICMS ST", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 765, nLine+469, 807)
	oDanfe:Say(nLine+462, 765 + 2,"VALOR IPI", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 807, nLine+469, 840)
	oDanfe:Say(nLine+468, 807 + 2,"ICMS", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 840, nLine+469, 870)
	oDanfe:Say(nLine+468, 840 + 2,"IPI", oFont08N:oFont)
	
	oDanfe:Box(nLine+452, 807, nLine+461, 870)
	oDanfe:Say(nLine+460, 807 + 2,"ALIQUOTA", oFont08N:oFont)
	
	For Nx :=1 to Len(aColProd)
		If nx==1
			oDanfe:Box(nLine+469,100,nLine+575,100)
		ElseIf  nx==2
			oDanfe:Box(nLine+469,145,nLine+575,145)
		ElseIf  nx==3
			oDanfe:Box(nLine+469,356,nLine+575,356)
		ElseIf  nx==4
			oDanfe:Box(nLine+469,391,nLine+575,391)
		ElseIf  nx==5
			oDanfe:Box(nLine+469,410,nLine+575,410)
		ElseIf  nx==6
			oDanfe:Box(nLine+469,433,nLine+575,433)
		ElseIf  nx==7
			oDanfe:Box(nLine+469,453,nLine+575,453)
		ElseIf  nx==8
			oDanfe:Box(nLine+469,499,nLine+575,499)
		ElseIf  nx==9
			oDanfe:Box(nLine+469,547,nLine+575,547)
		ElseIf  nx==10
			oDanfe:Box(nLine+469,593,nLine+575,593)
		ElseIf  nx==11
			oDanfe:Box(nLine+469,630,nLine+575,630)
		ElseIf  nx==12
			oDanfe:Box(nLine+469,675,nLine+575,675)
		ElseIf  nx==13
			oDanfe:Box(nLine+469,715,nLine+575,715)
		ElseIf  nx==14
			oDanfe:Box(nLine+469,765,nLine+575,765)
		ElseIf  nx==15
			oDanfe:Box(nLine+469,807,nLine+575,807)
		ElseIf  nx==16
			oDanfe:Box(nLine+469,840,nLine+575,840)
		EndIf
	Next
Else
	If nPosMsg > 0
		...
		nAux := nBaseCol + 30
		oDanfe:Box(nLine+452,nBaseCol+30,398,MAXBOXH+70)
	...
	Else
		...
		//		oDanfe:Box(nLine+452,nBaseCol+30,nLine+675,MAXBOXH+70)
		oDanfe:Box(nLine+454,nBaseCol+30,600,MAXBOXH+70)
		
		oDanfe:Box(nLine+454, 100, nLine+469, 145)
		oDanfe:Say(nLine+462, 100 + 2,"COD. PROD", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 145, nLine+469, 356)
		oDanfe:Say(nLine+462, 145 + 2,"DESCR PROD", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 356, nLine+469,391 )
		oDanfe:Say(nLine+462, 356 + 2,"NCM/SH", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 391, nLine+469, 410)
		If cMVCODREG == "1"
			oDanfe:Say(nLine+462, 391 + 2,"CSOSN", oFont08N:oFont)
		Else
			oDanfe:Say(nLine+462, 391 + 2,"CST", oFont08N:oFont)
		...
		oDanfe:Box(nLine+454, 410, nLine+469, 433)
		oDanfe:Say(nLine+462, 410 + 2,"CFOP", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 433, nLine+469, 453)
		oDanfe:Say(nLine+462, 433 + 2,"UN", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 453, nLine+469, 499)
		oDanfe:Say(nLine+462, 453 + 2,"QUANT.", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 499, nLine+469, 547)
		oDanfe:Say(nLine+462, 499 + 2,"V.UNITARIO", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 547, nLine+469, 593)
		oDanfe:Say(nLine+462, 547 + 2,"VLR TOTAL", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 593, nLine+469, 630)
		oDanfe:Say(nLine+462, 593 + 2,"BC.ICMS", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 630, nLine+469, 675)
		oDanfe:Say(nLine+462, 630 + 2,"BC.ICMS ST", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 675, nLine+469, 715)
		oDanfe:Say(nLine+462, 675 + 2,"VLR ICMS", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 715, nLine+469, 765)
		oDanfe:Say(nLine+462, 715 + 2,"VLR ICMS ST", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 765, nLine+469, 807)
		oDanfe:Say(nLine+462, 765 + 2,"VALOR IPI", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 807, nLine+469, 840)
		oDanfe:Say(nLine+468, 807 + 2,"ICMS", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 840, nLine+469, 870)
		oDanfe:Say(nLine+468, 840 + 2,"IPI", oFont08N:oFont)
		
		oDanfe:Box(nLine+454, 807, nLine+461, 870)
		oDanfe:Say(nLine+460, 807 + 2,"ALIQUOTA", oFont08N:oFont)
		
		For Nx :=1 to Len(aColProd)
			If nx==1
				oDanfe:Box(nLine+469,100,MAXBOXV,100)
			ElseIf  nx==2
				oDanfe:Box(nLine+469,145,MAXBOXV,145)
			ElseIf  nx==3
				oDanfe:Box(nLine+469,356,MAXBOXV,356)
			ElseIf  nx==4
				oDanfe:Box(nLine+469,391,MAXBOXV,391)
			ElseIf  nx==5
				oDanfe:Box(nLine+469,410,MAXBOXV,410)
			ElseIf  nx==6
				oDanfe:Box(nLine+469,433,MAXBOXV,433)
			ElseIf  nx==7
				oDanfe:Box(nLine+469,453,MAXBOXV,453)
			ElseIf  nx==8
				oDanfe:Box(nLine+469,499,MAXBOXV,499)
			ElseIf  nx==9
				oDanfe:Box(nLine+469,547,MAXBOXV,547)
			ElseIf  nx==10
				oDanfe:Box(nLine+469,593,MAXBOXV,593)
			ElseIf  nx==11
				oDanfe:Box(nLine+469,630,MAXBOXV,630)
			ElseIf  nx==12
				oDanfe:Box(nLine+469,675,MAXBOXV,675)
			ElseIf  nx==13
				oDanfe:Box(nLine+469,715,MAXBOXV,715)
			ElseIf  nx==14
				oDanfe:Box(nLine+469,765,MAXBOXV,765)
			ElseIf  nx==15
				oDanfe:Box(nLine+469,807,MAXBOXV,807)
			ElseIf  nx==16
				oDanfe:Box(nLine+469,840,MAXBOXV,840)
			EndIf
		Next
		nLine -= 257
	EndIf
