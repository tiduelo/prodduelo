...
Private _cNomFan    := ""
Private _nBsFCPSt   := 0
Private _nVlrFCPSt  := 0
Private _cCodCli    := ""//SA1->A1_COD+SA1->A1_LOJA

Private _cCliEnd	:= " "
Private _cCliCpl	:= " "
Private _dDataSai	:= ctod("//")
Private _cMsgAdc	:= ""
Private _aTbAdc		:= {}
Private _cSA1SEG	:= ""
Private _aPrdAdc	:= {}
private _npisbfria	:= 0 //variável para receber o valor do pis para bebidas frias - 23/02/2016
private _ncofbfria	:= 0 //variável para receber o valor do cofins para bebidas frias - 23/02/2016
Private _nPos		:= 0
Private _aPISCOF	:= { 0.00 , 0.00 }
Private _lSuframa	:= .F.
Private _cValFrete	:= 0.00
Private _nVlFrt   	:= 0.00
Private _cMotZNFDeso := ""

// Montagem da Tabela dos Percentuais conforme Segmento do Cliente
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		cVendName   := Posicione("SA3", 1, xFilial("SA3")+cVendedor,"A3_NREDUZ")
		_cCarga     := SF2->F2_CARGA
		_nBsFCPSt  := SF2->F2_BSFCPST
		////////////////////////////////////////////////////////////
			...
			If MsSeek(xFilial("SF3")+cClieFor+cLoja+cNota+cSerie)
				...
				// Duelo
				////////////////////////////////////////////////////////////
				_cNomFan    := alltrim(upper(SA1->A1_NREDUZ))
...
//				_cCodCli   := SA1->A1_COD
				_cCliEnd   := SA1->A1_END
				//_cCliCpl   := SA1->A1_COMPLEM
				_cCliCpl   := SA1->A1_PTREF //INCLUIDO POR RONALDO EM 19.09.2017 PARA ATENDER O PROCESSO DA NOVA ROTEIRIZAÇÃO
				////////////////////////////////////////////////////////////					
					If !Empty(SA1->A1_MENSAGE) .and. SA1->(Formula(A1_MENSAGE)) <> Nil .and. !EMPTY(SA1->(Formula(A1_MENSAGE)))
						...
						EndIf
					EndIf
				/* RAFAEL ALMEIDA - SIGACORP 14/06/2017  COMENTADO APEDIDO DO RONALDO
				////////////////////////////////////////////////////////////
				// Duelo
				////////////////////////////////////////////////////////////
				// Inclusao da Mensagem de Numero do Suframa
				If !Empty(SA1->A1_SUFRAMA) .And. SA1->A1_CALCSUF = 'I'
					cMensCli	+= " Codigo SUFRAMA (Cliente) - "+Alltrim(SA1->A1_SUFRAMA)+" "
					_lSuframa	:= .T.
				Endif
				////////////////////////////////////////////////////////////
				*/
				...
				// Duelo
				////////////////////////////////////////////////////////////
				_cNomFan   := alltrim(upper(SA2->A2_NREDUZ))
				_cCliEnd   := SA2->A2_END
				_cCliCpl   := SA2->A2_COMPLEM
				////////////////////////////////////////////////////////////
					aDest := {}
				...
				//if _lduelo .and. empty(SF2->F2_VEICUL1)
				if empty(SF2->F2_VEICUL1)
					
					aadd(aVeiculo,SF2->F2_PLACA)
					aadd(aVeiculo,SF2->F2_UFPLACA)
					aadd(aVeiculo,SA4->A4_ANTT) //RNTC
			...
			EndIf
				...
				////////////////////////////////////////////////////////////
			// Duelo - Incluir D2_GRUPO e D2_VALICM na pesquisa SQL
			////////////////////////////////////////////////////////////
				...
				#IFDEF TOP
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_GRUPO,D2_ITEM,D2_CF,
								...
								D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_BRICMSO,D2_ICMRETO,D2_BRICMSD,D2_ICMRETD,
								D2_GRUPO,D2_VALICM,D2_VALIMP5,D2_VALIMP6,D2_MARGEM %Exp:cField% 
								FROM %Table:SD2% SD2
								WHERE
					...
					Else
						BeginSql Alias cAliasSD2
							...
							D2_VALICM,D2_VALIMP5,D2_VALIMP6,D2_MARGEM %Exp:cField% 
							FROM %Table:SD2% SD2
							WHERE
							SD2.D2_FILIAL  = %xFilial:SD2% AND
							SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						/*If	SD2->(FieldPos("D2_DESCZFR"))<>0
							nValDscZF += (cAliasSD2)->D2_DESCZFR					
						EndIf */
					...
					dbSelectArea("SX5")
  					dbSetOrder(1)
					If MsSeek(xFilial("SX5")+"24"+SC5->C5_FORMPAG)
	  					cFormPgto := ALLTRIM(SX5->X5_DESCRI)//Alltrim(X5Descri())
					Endif
					////////////////////////////////////////////////////////////
					...
					If _lduelo
						...
						_dDataSai  := SF2->F2_SAIDA
						
						cMensDue := ""
						If !EMPTY(SF2->F2_MENS1)
							cMensDue += CRLF+AllTrim(SF2->F2_MENS1)
						Endif
						If !EMPTY(SF2->F2_MENS2)
							cMensDue += CRLF+AllTrim(SF2->F2_MENS2)
						Endif
						If !EMPTY(SF2->F2_MENS3)
							cMensDue += CRLF+AllTrim(SF2->F2_MENS3)
						Endif
						If !EMPTY(SF2->F2_MENS4)
							cMensDue += CRLF+AllTrim(SF2->F2_MENS4)
						Endif
						If !EMPTY(SF2->F2_MENS5)
							cMensDue += CRLF+AllTrim(SF2->F2_MENS5)
						Endif
						If !EMPTY(SF2->F2_MENS6)
							cMensDue += CRLF+AllTrim(SF2->F2_MENS6)
						Endif
						If !EMPTY(SF2->F2_MENS7)
							cMensDue += CRLF+AllTrim(SF2->F2_MENS7)
						Endif
						If !EMPTY(SF2->F2_PEDCLI)
							cMensDue += CRLF+"SEU PEDIDO : "+SF2->F2_PEDCLI
						Endif
						/*If !EMPTY(_cCliCpl)
							cMensDue += CRLF+"OBS: "+alltrim(upper(_cCliCpl))
						Endif*/
						If !EMPTY(_cCliCpl)
							cMensDue += CRLF+"PTO REFERENCIA: "+alltrim(upper(_cCliCpl))
						Endif
						If !AllTrim(cMensDue) $ cMensCli
							cMensCli += cMensDue
						EndIf
					EndIf
					...
					If Empty(SC6->C6_DESCRI)
						cDescri:= If(Empty(SB1->B1_DESCNF), SB1->B1_DESC  , SB1->B1_DESCNF) 		// SB1->B1_DESC
					Else
						cDescri:= If(Empty(SB1->B1_DESCNF), SC6->C6_DESCRI, SB1->B1_DESCNF)
					Endif
					
					cDescProd :=  alltrim(cDescProd)
					
					if ! empty( (cAliasSD2)->D2_LOTECTL )
						cDescProd += "|L:"+ alltrim( (cAliasSD2)->D2_LOTECTL )
					Endif
					       
					If !((cAliasSD2)->D2_TIPO $ "D")
						
						If Alltrim((cAliasSD2)->D2_GRUPO) $ "0204|0205|0206|0203"
							
							_nPisbfria += (cAliasSD2)->D2_VALIMP6
							_nCofbfria += (cAliasSD2)->D2_VALIMP5
							
							If Alltrim((cAliasSD2)->D2_GRUPO) $ "0204|0206" //Controle produtos vendidos suframa revisar para novos conceitos 23/02/2016
								
								Aadd(_aPrdAdc,	{(cAliasSD2)->D2_COD,;
								cDescri,;
								IIf(!(cAliasSD2)->D2_TIPO$"IP", IIf(!(cAliasSD2)->D2_TIPO $ "D", (cAliasSD2)->D2_PRCVEN+nDesconto,(cAliasSD2)->D2_PRCVEN), 0),;
								Posicione("SB1",1,xFilial("SB1")+(cAliasSD2)->D2_COD,"B1_CONV"),;
								Round((cAliasSD2)->D2_VALIPI/(cAliasSD2)->D2_QUANT,2),;
								Round((cAliasSD2)->D2_VALICM/(cAliasSD2)->D2_QUANT,2),;
								(cAliasSD2)->D2_QUANT,;
								Posicione("SB1",1,xFilial("SB1")+(cAliasSD2)->D2_COD,"B1_SEGUM"),;
								Round((cAliasSD2)->D2_VALIMP6/(cAliasSD2)->D2_QUANT,2),; //Posicione("SB1",1,xFilial("SB1")+(cAliasSD2)->D2_COD,"B1_VLR_PIS"),;
								Round((cAliasSD2)->D2_VALIMP5/(cAliasSD2)->D2_QUANT,2),; //Posicione("SB1",1,xFilial("SB1")+(cAliasSD2)->D2_COD,"B1_VLR_COF"),;
								0.00,;
								(cAliasSD2)->D2_VALFRE,;
								(cAliasSD2)->D2_DESCZFR,;
								(cAliasSD2)->D2_MARGEM,;//RAFAEL ALMEIDA (14/06/2017)
								(cAliasSD2)->D2_TOTAL}) //RAFAEL ALMEIDA (27/07/2017)								
							Endif
				...
				// Msg Zona Franca de Manaus / ALC
					...
					If !SF2->F2_DESCZFR == 0 .or. ( lInfAdZF .and. nValBse > 0 )//Desnecessario seek redundante na SF3 pois o campo F2_DESCZFR ja possui os valores de ZFR de toda a venda
					    ...
					    Else
							...
							// Duelo
							////////////////////////////////////////////////////////////
							//cMensFis += "Total do desconto Ref. a Zona Franca de Manaus / R$ "+ str(nValDscZF,13,2)
							////////////////////////////////////////////////////////////
					    	//cMensFis += "Total do desconto Ref. a Zona Franca de Manaus / ALC. R$ "+str(nValBse-SF2->F2_DESCONT,13,2)
					    EndIF
					EndIf 	
...
If aNota[5]=="D"
	...
	_cVendDev := GetAdvFVal("SF2", "F2_VEND1" ,xFilial("SF2")+ SF1->F1_FORNECE + SF1->F1_LOJA + aNfVinc[1][3] + aNfVinc[1][2] ,2)
	cVendName := GetAdvFVal("SA3", "A3_NREDUZ",xFilial("SA3")+ _cVendDev,1)
	_cCodCli  := SF1->F1_FORNECE+"/"+SF1->F1_LOJA
	_cNomFan  := ALLTRIM(UPPER(GetAdvFVal("SA1", "A1_NREDUZ",xFilial("SA1")+ SF1->F1_FORNECE + SF1->F1_LOJA ,1)))

EndIf

                                                                        
//iif(!empty(_cCarga), +" | COD/CARGA: " +_cCarga, "")

//cMensCli += " | FORMA PGTO: "	+cFormPgto + " | VENDEDOR: "		+cVendName + iif(!empty(_cNomFan), +" | COD/LOJA: " +_cCodCli + " | NOME FANTASIA: "+_cNomFan, "")+" | "+iif(!empty(_cCarga), +"CARGA: " +_cCarga, "")+" | " 
//cMensCli += iif( aNota[5]=="D", " | VENDEDOR: " , " | FORMA PGTO: "	+cFormPgto + " | VENDEDOR: " )	+cVendName + iif(!empty(_cNomFan), +" | COD/LOJA: " +_cCodCli + " | NOME FANTASIA: "+_cNomFan, "")+" | "+iif(!empty(_cCarga), +"CARGA: " +_cCarga, "")+" | "
cMensCli += iif( aNota[5]=="D", " | VENDEDOR: " , " | FORMA PGTO: "	+cFormPgto + " | VENDEDOR: " )	+cVendName + iif(!empty(_cNomFan), +" | COD/LOJA: " +_cCodCli + " | NOME FANTASIA: "+_cNomFan, "")+" | "+iif(!empty(_cCarga), +"CARGA: " +_cCarga, "")+" | "+iif(_nBsFCPSt > 0, +"Base de Calculo - FCP na UF de destino: R$ "+ConvType(_nBsFCPSt,15,2)+", Valot Total  - FCP na UF de destino: R$ "+ConvType(_nVlrFCPSt,15,2)+". ", "")+" | " 

//cMensCli += " | FORMA PGTO: "	+cFormPgto + " | VENDEDOR: "		+cVendName + iif(!empty(_cNomFan), +" | COD/LOJA: " +_cCodCli + " | NOME FANTASIA: "+_cNomFan, "")+" | " 
//cMensCli += " | FORMA PGTO: "	+cFormPgto + " | VENDEDOR: "		+cVendName + iif(!empty(_cNomFan), + " | NOME FANTASIA: "+_cNomFan, "")+" | "

//Modificado por Dias 05.03.2012 - Inclusao de Mensagem de Preco Sugerido conforme Cliente

//Colunas do Array _aPrdAdc
// 1 - Codigo do Produto
// 2 - Descricao do Produto
// 3 - Valor Unitario
// 4 - Conversao - B1_CONV
// 5 - Valor Icms da Nota / Quant
// 6 - Valor IPI da Nota / Quant
// 7 - Quantidade
// 8 - Segunda Unidade de Medida
// 9 - Valor PIS
// 10 - Valor COFINS
// 11 - Valor Sugerido
// 12 - Valor do Frete
// 13 - Valor Desconto de Suframa
// 14 - Valor Margem 
// 15 - Valor Total do Item
// ***********************************************************
// Calculo
// ***********************************************************
// 1 - Preco Individual / Fator de Conversao - Preco de Venda por Unidade de Produto
// 2 - (Valor do Ipi / Quantidadade) / Fator de Conversao - Preco do IPI por Unidade de Produto
// 3 - Somar (1 + 2) + Percentual do Segmento conforme Cadastro do Cliente (A1_SATIV1)
// 4 - Resultado sera o Preco Sugerido
// ***********************************************************
...
// Preenchimento do Preco Sugerido para a Mensagem

If _nPos > 0 .And. Len(_aPrdAdc) > 0
	
	if sm0->m0_codigo == "04" // Duelo
		
		If SA1->A1_EST = GetMv("MV_ESTADO")
		//	cMensCli += " PRECO SUG.CONSUMIDOR-ART.39,8.,LEI 5530/89 - RICMS/PA / " 
			cMensCli += " Refrigerante - Base de cálculo da substituição tributária conforme Portaria n.º 17/2016"	
		ElseIf SA1->A1_EST = "AP"
			cMensCli += " PRECO SUG.CONSUMIDOR-ART.258,§3o.,DEC 2269/98 - RICMS/AP / "
		Endif
		
//		_nPerc 	:= 1+(_aTbAdc[_nPos,2]/100)// RAFAEL ALMEIDA - SIGACORP (14/06/2017) COMENTADO POIS HOUVE MUDANÇA CALCULO VALOR SUGERIDO

		
		lFirst 	:= .T.
		
		For _nx := 1 to Len(_aPrdAdc)

			_nPerc		:= 1+(_aPrdAdc[_nx,14]/100)// RAFAEL ALMEIDA - SIGACORP (14/06/2017) ADICIONADO CALCULO VALOR SUGERIDO
			_nVlSug 	:= 0.00
			
			If _aPrdAdc[_nx,3] > 0.00
				
				If SA1->A1_EST = "AP"
					_nVlSug := ((((_aPrdAdc[_nx,15]+_aPrdAdc[_nx,12]-_aPrdAdc[_nx,13])/_aPrdAdc[_nx,7])/_aPrdAdc[_nx,4])*_nPerc)// RAFAEL ALMEIDA - SIGACORP (14/06/2017) ADICIONADO CALCULO VALOR SUGERIDO
				//	_nVlSug := ((_aPrdAdc[_nx,3]+(_aPrdAdc[_nx,12]/_aPrdAdc[_nx,7]))/_aPrdAdc[_nx,4])*_nPerc // RAFAEL ALMEIDA - SIGACORP (14/06/2017) COMENTADO POIS HOUVE MUDANÇA CALCULO VALOR SUGERIDO
				//Else
				//	_nVlSug := ((_aPrdAdc[_nx,3]+_aPrdAdc[_nx,5])/_aPrdAdc[_nx,4])*_nPerc
				Endif
			Endif
			
			_aPrdAdc[_nx,11] := Round(_nVlSug,2)
			
			_aPISCOF[1] +=   Round(_aPrdAdc[_nx,9] *_aPrdAdc[_nx,7],2)
			_aPISCOF[2] +=	 Round(_aPrdAdc[_nx,10]*_aPrdAdc[_nx,7],2)
			 
			If SA1->A1_EST = "AP"
			cMensCli += If(lFirst, "","; ")+Alltrim(_aPrdAdc[_nx,2])+"("+Alltrim(Transform(Round(_nVlSug,2),"@E 999,999.99"))+") "
			EndIf
...
If _lSuframa	//Duelo
	...
	If _aPISCOF[1] > 0.00 .And. _aPISCOF[2] > 0.00	//Duelo
		cMensCli += " PIS/CONFINS CONFORME LEI 10.833/03 ("+SPACE(03)
		cMensCli += " PIS R$ "+Alltrim(Transform(Round(_aPISCOF[1],2),"@E 999,999.99"))+" "
		cMensCli += " COFINS R$ "+Alltrim(Transform(Round(_aPISCOF[2],2),"@E 999,999.99"))+" )"
		
	Endif

Else

	If  _nPisbfria > 0 .And. _nCofbfria > 0	//Duelo - Verificar se suframa vai pra bebida fria
		cMensCli += Chr(13) + Chr(10) + " Bebidas Frias: Vlr Pis: R$ " + Alltrim(Transform(Round(_nPisbfria,2),"@E 999,999.99"))+;
		" Vlr Cofins: R$ " + Alltrim(Transform(Round(_nCofbfria,2),"@E 999,999.99"))
	Endif
...
If cVeramb >= "3.10"
	...
	// Duelo
	////////////////////////////////////////////////////////////	
cString += NfeTag('<dhSaiEnt>',IIf(!empty(_dDataSai), ConvType(_dDataSai) +"T" + IIf(Len(aNota[06]) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00") , ConvType(aNota[03])+"T"+IIf(Len(aNota[06]) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
Else	
...
If cVerAmb >= "3.10" .and. Len(aICMSZFM) > 0 .And. Len(aCST) > 0 .And. !Empty(aICMSZFM[1])
	If !(lMvNFLeiZF)	
		...
		cString += NfeTag('<vDesc>' ,ConvType((aProd[31]+aProd[32])+aProd[15]+aProd[26],15,2)) // duelo - incluído aprod[26]			
	Else	
	...
	If Len(aIcmsST)>0	
		...
		If Len(aICMSZFM)>0 .And. aCST[1] $ '30-40'
			if !empty( aICMSZFM[02] )
				...
				_cMotZNFDeso := ConvType(aICMSZFM[02])
			endif	
...
If nVicmsDeson >0  
//INICIO - Duelo ICMS convenio do ICMS 134/2019
If Alltrim(_cMotZNFDeso) == "1"
	_cMoticmsdeson := "1 - Táxi"
ElseIf Alltrim(_cMotZNFDeso) == "2"
	_cMoticmsdeson := "2 - Deficiente Físico"
ElseIf Alltrim(_cMotZNFDeso) == "3"
	_cMoticmsdeson := "3 - Produto agropecuário/Uso na agropecuária"
ElseIf Alltrim(_cMotZNFDeso) == "4"
	_cMoticmsdeson := "4 - Frotista/Locadora"
ElseIf Alltrim(_cMotZNFDeso) == "5"
	_cMoticmsdeson := "5 - Diplomático/Consular"
ElseIf Alltrim(_cMotZNFDeso) == "6"
	_cMoticmsdeson := "6 - Utilitários e Motocicletas da Amazônia Ocidental e Áreas de Livre Comércio"
ElseIf Alltrim(_cMotZNFDeso) == "7"
	_cMoticmsdeson := "7 - SUFRAMA"
ElseIf Alltrim(_cMotZNFDeso) == "8"
	_cMoticmsdeson := "8 - Venda a Órgão Público"
ElseIf Alltrim(_cMotZNFDeso) == "9"
	_cMoticmsdeson := "9 - Outros"
ElseIf Alltrim(_cMotZNFDeso) == "10"
	_cMoticmsdeson := "10 - Deficiente Condutor"
ElseIf Alltrim(_cMotZNFDeso) == "11"
	_cMoticmsdeson := "11 - Deficiente Não Condutor"
ElseIf Alltrim(_cMotZNFDeso) == "12"
	_cMoticmsdeson := "12 - Órgão de fomento e desenvolvimento agropecuário"
ElseIf Alltrim(_cMotZNFDeso) == "16"
	_cMoticmsdeson := "16 - Olimpiadas Rio 2016"
ElseIf Alltrim(_cMotZNFDeso) == "90"
	_cMoticmsdeson := "90 - Solicitado pelo Fisco"
Else
	_cMoticmsdeson := "Não Cadastrado"
EndIf
	cString += "Motivo da Desoneracao do ICMS: "+ Alltrim(_cMoticmsdeson) +". "

//FIM -  - Duelo ICMS convenio do ICMS 134/2019
