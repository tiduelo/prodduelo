...
Local aOrdem   := {"Produto",STR0005,STR0006,STR0007}//"Fornecedor"###"Data De Digitacao"###"Tipo+Grupo+Codigo"###" Grupo+Codigo"
Local aArea	   := Getarea() 
...
#IFDEF TOP
	...
	cSelect1 := "D1_FILIAL, D1_DOC, D1_SERIE, D1_FORNECE, D1_LOJA, D1_DTDIGIT, D1_COD,   D1_QUANT, D1_VUNIT, D1_PEDIDO, D1_ITEMPC,"
	...
	SELECT D1_FILIAL,D1_DOC,     D1_SERIE,   D1_FORNECE, D1_LOJA, D1_DTDIGIT,    D1_COD,   D1_QUANT, D1_VUNIT, D1_PEDIDO, D1_ITEMPC,
...
PRIVATE tamanho := "G"
PRIVATE aOrd    := {OemToAnsi(" Produto"),OemToAnsi(STR0005),OemToAnsi(STR0006) ,OemToAnsi(STR0007)}//" Fornecedor         "###" Data De Digitacao  "###" Tipo+Grupo+Codigo  "###" Grupo+Codigo   "
...
#IFDEF TOP
	If (TcSrvType()#'AS/400')
	  ...
	  If nOrdem == 1
	   	 cQuery += " ORDER BY 7,6" //" ORDER BY 4,5,2,3,20"	 //D1_FORNECE,D1_LOJA,D1_DOC,D1_SERIE, D1_ITEM	
	  ElseIf nOrdem == 2 
...
#ENDIF        
		...
		If nOrdem == 1
			...
			Condicao += " .And. D1_COD <= '" + mv_par02 + "'"
			cIndice  := "D1_COD+D1_DTDIGIT"
		ElseIf nOrdem == 2
...
While !Eof()
	...
	If lFiltro
		...
		@ li,097 PSAY (cAliasSD1)->D1_IPI  Picture PesqPict("SD1","D1_IPI",5)   //    MACEDO -- JMC -- CBAA
      ...
      DBSELECTAREA("SC7")
      DBSETORDER(1)
      DBSEEK(xFilial("SC7")+(cAliasSD1)->D1_PEDIDO+(cAliasSD1)->D1_ITEMPC)
      IF FOUND()
         Li++
         @ li,029+nIncCol PSAY "Obs: "+C7_OBS
         @ LI,126         PSAY "Ped: "+(cAliasSD1)->D1_PEDIDO+"/"+(cAliasSD1)->D1_ITEMPC+" Dt.Ped: "+DTOC(C7_EMISSAO)
         DBSELECTAREA("SCR")
         DBSEEK(SC7->C7_FILIAL+"PC"+SC7->C7_NUM)
         IF FOUND()
            _LIBERADOR:=CR_USERLIB
            _DATALIB  :=CR_DATALIB
           ELSE
            _LIBERADOR:=SPACE(6)
            _DATALIB  :=CTOD("  /  /  ")
         ENDIF   
         @ LI,164 PSAY " // Dt Liberacao: "+DTOC(_DATALIB)+" Por: "+_LIBERADOR
      ENDIF
	...
	Eof()
		...
		If nOrdem == 1 .Or. nOrdem == 2
			...
			If nCnt >= 1
				...
				If nOrdem == 2 .and. (cAliasSD1)->D1_FORNECE <> cForna
					ImpTotF(nTotForn,titulo,nOrdem,nTotQt1,nTotVal1,aTotais,nIncCol,cQUEBRA)
					...
					nTotVal1   := 0
				Endif
				If nOrdem == 1 .and. (cAliasSD1)->D1_COD <> cQuebra 
					ImpTotF(nTotForn,titulo,nOrdem,nTotQt1,nTotVal1,aTotais,nIncCol,cQUEBRA)
					aTotais[3] := 0 // Total imposto nao incluido / Fornecedor
					aTotais[4] := 0 // Total imposto incluido     / Fornecedor
					nTotForn   := 0
...
IF nORDEM=1    // NAO IMPRIME TOTAL DA NF PARA ORDEM DE PRODUTO
   RETURN 
ENDIF   
If li > 59
	cabec(titulo+" ("+alltrim(aOrd[nOrdem])+")",cabec1,cabec2,nomeprog,tamanho,15)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function ImpTotF(nTotProd,titulo,nOrdem,nTotQt1,nTotVal1,aTotais,nIncCol,cQUEBRA)
If li > 59
	cabec(titulo+" ("+alltrim(aOrd[nOrdem])+")",cabec1,cabec2,nomeprog,tamanho,15)
Endif
